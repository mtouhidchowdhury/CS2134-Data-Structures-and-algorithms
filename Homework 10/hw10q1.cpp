...
bool HashTable<HashedObj>::insert(const HashedObj & x) {
	if (find(x) != array.end()) { return false; } //already exist
	int hashNum = hf(x) % array.size();
	while (array[hashNum].info == ACTIVE) {
		hashNum = (hashNum + 1) % array.size();
	}
	array[hashNum].element = x;
	array[hashNum].info = ACTIVE;
...
typename HashTable<HashedObj>::iterator HashTable<HashedObj>::find(const HashedObj & x) {
	...
	while (arraySize < array.size()) {
		if (array[hashNum].info == DELETED) {
			hashNum = (hashNum + 1) % array.size();
			arraySize++;
		}else if (array[hashNum].info == EMPTY) { return array.end(); }
		else {
			if (array[hashNum].element == x) { return array.begin() + hashNum; }
			hashNum = (hashNum + 1) % array.size();
			arraySize++;
		}
	} return array.end();
