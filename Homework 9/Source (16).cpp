	...
	*/
	...
	void levelOrderTraversal() const {
		...
		while (aQueue.empty() == false) {
			Node* ToPrint = aQueue.front();
			cout << ToPrint->element << endl;
			aQueue.pop();
			if (ToPrint->left != nullptr) { aQueue.push(ToPrint->left); }
			if (ToPrint->right != nullptr) { aQueue.push(ToPrint->right); }
		}
	}
	

	double average_node_depth(Node* root, int& size, int depth) {
		if (root == nullptr) { return 0; }
		size = size + 1;
		return depth + average_node_depth(root->left, size,depth+1) + 
			average_node_depth(root->right, size, depth+1);
	}

	double average_node_depth() {
		int size = 0;
		double totalDepth = average_node_depth(root, size, 0);
		return totalDepth / size;
	}


	void rangedPrint(int low, int high)  {
		rangedPrint(root, low, high);
	}
	void rangedPrint(Node* x, int low, int high) {
		if (x == nullptr) { return; } //reached end
		if (x->element > low && x->element < high) { //within range 
			rangedPrint(x->left, low, high);
			cout << x->element;
			rangedPrint(x->right, low, high);
		}else if (x->element > high) { // too big
			rangedPrint(x->left, low, high);
		}
		else {
			rangedPrint(x->right, low, high);
	...
	void stringy() {
		stack<Comparable> aStack;
		aStack.push(root->element);
...
private:
	...
	*/
	...
	bool contains(const Comparable& x, Node* t) const {
		if (t == nullptr) { return false; } // reached end without finding
		else if (t->element < x) { return contains(x, t->right); } //element greater so call the great subtree
		else if (x < t->element) { return contains(x, t->left); } //element less so call left side
		return true; // if didnt reach null return true 
	}
